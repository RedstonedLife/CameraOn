...
public class $UI$01 {
    ...
    public static void u_01$1$1(SplitPane csplitpane, Button[] buttons) {
        u_00$1$1(csplitpane);
        buttons[0].setOnAction(e -> MainClass.getSCENE_MANAGER().changeDialogScene("mainWindow",MainClass.class.getResource(FrontendPaths.SETTINGS_FXML)));
        buttons[1].setOnAction(e -> MainClass.getSCENE_MANAGER().changeDialogScene("mainWindow",MainClass.class.getResource(FrontendPaths.CAM_SETTINGS)));
        buttons[2].setOnAction(e -> MainClass.getSCENE_MANAGER().changeDialogScene("mainWindow",MainClass.class.getResource(FrontendPaths.APPLY_FXML)));
        buttons[3].setOnAction(e -> MainClass.getSCENE_MANAGER().changeDialogScene("mainWindow",MainClass.class.getResource(FrontendPaths.SCAN_FXML)));
        buttons[4].setOnAction(e -> {if(MainClass.getSCENE_MANAGER().getDialog("popUpWindow").isEnabled()) {MainClass.getSCENE_MANAGER().getDialog("popUpWindow").closeDialog();} else {MainClass.getSCENE_MANAGER().getDialog("popUpWindow").openDialog();BNC.getSCENE_MANAGER().changeDialogScene("popUpWindow", MainClass.class.getResource(FrontendPaths.POP_HELP_FXML));}});
    }
