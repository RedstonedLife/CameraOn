...
public class $UI$01 {
    ...
    public static void u_11$1$4(TextField[] textFields, Rectangle[] rectangles) {
        if(licenseHandler.verifyProductKey($UI$01.u_11$1$2(textFields)) != ProductKeyState.KEY_GOOD) {
            ...
            try{
                if(MainClass.getRegi().get("isAT", null) == null || !MainClass.getRegi().get("isAT").equals(MainClass.getRegi().put("isAT", SHA512.getSecureSHA("true&&"+Launcher.getHu().getFileStores().get(0).getUUID(),MainClass.getRegi().get("ST", null).getBytes()));)) {
