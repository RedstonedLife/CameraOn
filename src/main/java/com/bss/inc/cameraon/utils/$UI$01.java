...
public class $UI$01 {
    ...
    public static void u_01$1$1(SplitPane csplitpane, Button[] buttons) {
        ...
        buttons[4].setOnAction(e -> {if(MainClass.getWindowManager().getDialog("popUpWindow").isEnabled()) {MainClass.getWindowManager().getDialog("popUpWindow").closeDialog();} else {MainClass.getWindowManager().getDialog("popUpWindow").openDialog();BNC.getWindowManager().changeDialogScene("popUpWindow", MainClass.class.getResource(FrontendPaths.POP_HELP_FXML));}});
    }
    ...
    public static void u_04$1$1(Button object) {object.setOnAction(e -> {
        BNC.getWindowManager().changeDialogScene("preferencesDialog", MainClass.class.getResource(FrontendPaths.PREF_DISPLAY_FXML)); // <--- Look at this closely;
        BNC.getWindowManager().openDialog("preferencesDialog");
    });}
