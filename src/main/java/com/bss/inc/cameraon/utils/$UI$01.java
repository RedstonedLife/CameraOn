...
public class $UI$01 {
    ...
    public static void u_07$1$1(Button[] buttons, Text fpsText, ChoiceBox[] cbs) throws FileNotFoundException {
        ...
        cbs[0].setOnAction(e -> {_j.setValue("videnc", cbs[0].getItems().indexOf(cbs[0].getValue())+1);try {Launcher.SettingsContainer.Save();} catch (FileNotFoundException ex) {ex.printStackTrace();}});
        cbs[1].setOnAction(e -> {_j.setValue("resolution", cbs[1].getItems().indexOf(cbs[1].getValue())+1);try {Launcher.SettingsContainer.Save();} catch (FileNotFoundException ex) {ex.printStackTrace();}});
        cbs[2].setOnAction(e -> {_j.setValue("bitrate", cbs[2].getItems().indexOf(cbs[2].getValue())+1);try {Launcher.SettingsContainer.Save();} catch (FileNotFoundException ex) {ex.printStackTrace();}});
    }
