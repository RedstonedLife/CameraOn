...
public abstract class ProductKeyGenerator<ED extends ProductKeyEncodingData> {
    ...
    public ProductKeyState verifyProductKey(final String productKey) {
        if((productKey != null) && (productKey.trim().length() > 0)) {
            ...
            }if ((this.productKeyEncodingData != null) && (this.productKeyEncodingData.length > 0)) {if (this.getProductKeySectionWorker() != null) {try {long seed = Long.parseLong(cleanedProductKey.substring(0, this.getSeedCharLength()), 16);int currentKeyCharIndex = this.getSeedCharLength();String tmpKeySection;for (int n = 0; n < this.productKeyEncodingData.length; n++) {if (this.productKeyEncodingData[n] != null) {tmpKeySection = cleanedProductKey.substring(currentKeyCharIndex, currentKeyCharIndex + 2);if (!tmpKeySection.equals(ProductKeyUtils.buildHexStr(2, this.getProductKeySectionWorker().buildProductKeySection(seed, this.productKeyEncodingData[n])))) return ProductKeyState.KEY_PHONY;}currentKeyCharIndex += 2;}return ProductKeyState.KEY_GOOD;} catch (NumberFormatException nfe) {Logger.getLogger("BNC").error("Exception caught", nfe);}}}}return ProductKeyState.KEY_PHONY;
